...
from store.graph_output_handlers.neo4j_output_handlers.relations_file import RelationsFile
from utils.logging import get_logger


def process_all_files_for_neo4j_import(matchMetadataFilepath, matchEventsFilepath, outputDirectory):
    Path(outputDirectory).mkdir(parents=True, exist_ok=True)
    logger = get_logger(outputDirectory=outputDirectory, fileName=f'football_graph_{datetime.now().date()}.log', overwriteExistingFile=True)
    try:
        nodeOutputHandler = NodesFile(fileName=f'{outputDirectory}/football_event_graph_nodes.csv.gz')
        relationOutputHandler = RelationsFile(fileName=f'{outputDirectory}/football_event_graph_relations.csv.gz')
        databaseBuilder = GraphDatabaseBuilder(nodeOutputHandler=nodeOutputHandler, relationsOutputHandler=relationOutputHandler)
        teamToIdMap = process_match_metadata_file(matchMetadataFilepath=matchMetadataFilepath, databaseBuilder=databaseBuilder, logger=logger)
        process_match_events_file(matchEventsFilepath=matchEventsFilepath, databaseBuilder=databaseBuilder, teamToIdMap=teamToIdMap, logger=logger)
...
def process_match_metadata_file(matchMetadataFilepath, databaseBuilder, logger):
    ...
    leagueToIdMap = {league: index for index, league in enumerate(set(matchMetadataDataframe['league']))}
    countryToIdMap = {country: index for index, country in enumerate(set(matchMetadataDataframe['country']))}
    seasonToIdMap = {season: index for index, season in enumerate(set(matchMetadataDataframe['season']))}
    teamToIdMap = {team: index for index, team in enumerate(set(matchMetadataDataframe['ht']).union(matchMetadataDataframe['at']))}
...
def process_match_events_file(matchEventsFilepath, databaseBuilder, teamToIdMap, logger):
    ...
    def _get_unique_names(names: iter):
    ...
    allPlayerNames = set.union(*(playerNames, player2Names, playerInNames, playerOutNames))
    ...
    databaseBuilder.add_football_events(remappedEventData=matchEventsDataframe, teamToIdMap=teamToIdMap, playerToIdMap=playerToIdMap)
