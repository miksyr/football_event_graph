...
class GraphDatabaseBuilder:
    ...
    def add_dates(self, allDates: iter):
        ...
        for year, month, day in splitDates:
            self.nodeOutputHandler.add(nodeId=DateId(year=year, month=month, day=day), nodeLabels=[NodeLabel.DATE], nodeProperties={NodeField.TEXT: f'{year}-{month}-{day}'})
            self.relationsOutputHandler.add(startNodeId=MonthId(year=year, month=month), endNodeId=YearId(year=year), relationType=GeneralRelationType.IN_YEAR)
            self.relationsOutputHandler.add(startNodeId=DateId(year=year, month=month, day=day), endNodeId=MonthId(year=year, month=month), relationType=GeneralRelationType.IN_MONTH)
    ...
    def add_football_matches(self, remappedMetadata: pd.DataFrame):
        remappedMetadata = replace_nan_with_none_in_dataframe(dataframe=remappedMetadata)
        lastMatchForTeam = {}
        for _, metadataRow in tqdm(remappedMetadata.iterrows(), total=len(remappedMetadata)):
            nodeProperties = {
            ...
            self.relationsOutputHandler.add(startNodeId=dateId, endNodeId=matchId, relationType=GeneralRelationType.ON_DATE)
            seasonId = SeasonId(seasonId=metadataRow['season'])
            self.relationsOutputHandler.add(startNodeId=seasonId, endNodeId=matchId, relationType=GeneralRelationType.IN_SEASON)

            homeTeamId = TeamId(teamId=metadataRow['ht'])
            awayTeamId = TeamId(teamId=metadataRow['at'])
            self.relationsOutputHandler.add(startNodeId=matchId, endNodeId=homeTeamId, relationType=GeneralRelationType.HOME_TEAM)
            self.relationsOutputHandler.add(startNodeId=matchId, endNodeId=awayTeamId, relationType=GeneralRelationType.AWAY_TEAM)
            previousHomeMatch = lastMatchForTeam.get(homeTeamId)
            if previousHomeMatch is not None:
                self.relationsOutputHandler.add(startNodeId=previousHomeMatch, endNodeId=matchId, relationType=GeneralRelationType.NEXT)
            previousAwayMatch = lastMatchForTeam.get(awayTeamId)
            if previousAwayMatch is not None:
                self.relationsOutputHandler.add(startNodeId=previousAwayMatch, endNodeId=matchId, relationType=GeneralRelationType.NEXT)

            leagueId = LeagueId(leagueId=metadataRow['league'])
            countryId = CountryId(countryId=metadataRow['country'])
            self.relationsOutputHandler.add(startNodeId=matchId, endNodeId=leagueId, relationType=GeneralRelationType.IN_LEAGUE)
            self.relationsOutputHandler.add(startNodeId=leagueId, endNodeId=countryId, relationType=GeneralRelationType.IN_COUNTRY)

    def add_assist_methods(self):
    ...
    def add_football_events(self, remappedEventData: pd.DataFrame, teamToIdMap: dict, playerToIdMap: dict):
        remappedEventData = replace_nan_with_none_in_dataframe(dataframe=remappedEventData)
        for _, row in tqdm(remappedEventData.iterrows(), total=len(remappedEventData)):
            ...
            self.relationsOutputHandler.add(startNodeId=matchId, endNodeId=matchEventId, relationType=GeneralRelationType.HAS_MATCH_EVENT)
            self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=TeamId(teamId=int(teamToIdMap[row['event_team']])), relationType=EventRelationType.EVENT_TEAM)
            self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=TeamId(teamId=int(teamToIdMap[row['opponent']])), relationType=EventRelationType.OPPONENT_TEAM)
            # player + player2 pairs are always distinct from playerIn + playerOut pairs
            if row['player'] is not None:
                self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=PlayerId(playerId=int(playerToIdMap[row['player']])), relationType=EventRelationType.PLAYER_1)
            if row['player2'] is not None:
                self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=PlayerId(playerId=int(playerToIdMap[row['player2']])), relationType=EventRelationType.PLAYER_2)
            if row['player_in'] is not None:
                self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=PlayerId(playerId=int(playerToIdMap[row['player_in']])), relationType=EventRelationType.PLAYER_1)
            if row['player_out'] is not None:
                self.relationsOutputHandler.add(startNodeId=matchEventId, endNodeId=PlayerId(playerId=int(playerToIdMap[row['player_out']])), relationType=EventRelationType.PLAYER_2)

            if row['shot_place'] is not None:
                shotPlacementId = int(row['shot_place'])
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToShotPlacementMap[shotPlacementId], eventId=shotPlacementId),
            ...
            if row['shot_outcome'] is not None:
                ...
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToShotOutcomeMap[shotOutcomeId], eventId=shotOutcomeId),
            ...
            if row['location'] is not None:
                ...
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToPitchLocationMap[pitchLocationId], eventId=pitchLocationId),
            ...
            if row['bodypart'] is not None:
                ...
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToBodyPartMap[bodyPartId], eventId=bodyPartId),
            ...
            if row['assist_method'] is not None:
                ...
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToAssistMethodMap[assistMethodId], eventId=assistMethodId),
            ...
            if row['situation'] is not None:
                ...
                self.relationsOutputHandler.add(
                    startNodeId=matchEventId,
                    endNodeId=EventContextId(eventType=idToEventSituationMap[eventSituationId], eventId=eventSituationId),
